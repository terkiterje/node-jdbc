...
Pool.prototype.terminate = function(callback) {
  var all=this._pool.concat(this._reserved);
  asyncjs.parallel(all.map(function(conn) {
    return function(cb) {
      if (!conn || !conn.conn || !conn.conn.close) {
        return cb(null);
      }
      conn.conn.close(function(err) {
        if (err) console.warn('Error closing connection: '+err);
        cb(null);
      });
    };
  }),callback);
